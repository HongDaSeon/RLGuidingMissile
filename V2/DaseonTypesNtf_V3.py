...
class randomTitle:
    def __init__(self):
        self.title = self.titlepicker()

    def titlepicker(self):
        adve = ['Very','Slightly','Badly','Friendly','Costly','Timely','Orderly','Lively','Highly','Mostly']
        adje = ['Funny','Confidential','Sensible','Hot','Cold','Foggy','Tall','Warm','Calm','Gentle','Sad','Angry']
        noun = ['Paper','Lab','Computer','Gold','Butter','Wine','Juice','Bread','Can','Bottle','Dragon','Cat']
        
        pick1 = adve[rd.randrange(0,len(adve))]
        pick2 = adje[rd.randrange(0,len(adje))]
        pick3 = noun[rd.randrange(0,len(noun))]
        pick4 = number = str(int(rd.random()*100))
